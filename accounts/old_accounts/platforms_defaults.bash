...
export QUEUENAME=pgc1
export WORKDIR=/proc1/efernandez
...
###############################################
###############################################
if [[ ${HOSTNAME} == *Geodesy1* ]]
then
  export PLATFORM_NAME=Geodesy1
  export JOBSCHEDULER=SLURM
  #export QUEUENAME=compute
  export QUEUENAME=pgc1
  export MAX_MEMORY_PER_NODE=500000
  export NUMBER_OF_CORES_PER_NODE=64
  export NUMBER_OF_THREADS_PER_CORE=1
  export JOBSHEDULER_PROJECTNAME=insar
  export WORKDIR=/proc1/$USER
  export SCRATCHDIR=/proc1/$USER/scratch
fi
###############################################
if [[ ${USER} == *circleci* ]] 
then
